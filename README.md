...
文字列はUTF-8の不変な値で、表示・等値比較・関数やデータ構造での受け渡しに対応します。現在は`check`・`emit-ir`・`emit-c`・`emit-bytecode`・`run`で使用できます。LLVM・QBE・WAT・アセンブリへの出力は未対応としてソース位置付きで診断します。連結や文字列の添字参照は未実装です。
