...
- **型と変数:** 静的型付け。`bool`、`i8`・`u8`・`i16`・`u16`・`i32`・`u32`・`i64`、`f32`・`f64`。型宣言、`infer`、不変な束縛と`mut`による可変な束縛。
...
`u64`、文字列、動的な長さの配列、再帰、失敗からの回復、明示的な丸め・切り捨て操作は未実装です。現在の生成先では小さい整数型も64ビット領域に格納し、値の範囲を検査します。
