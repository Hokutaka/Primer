...
要素型には`bool`、`i32`、`u32`、`i64`、`f32`、`f64`、名前付きproduct typeまたは固定長配列を使用できます。固定長配列はproduct typeのfieldにも使用できます。
...
関数のparameterと戻り値には`bool`、`i32`、`u32`、`i64`、`f32`、`f64`、名前付きproduct type、固定長配列を使えます。product typeと配列は値として渡されるため、関数が受け取った値と呼び出し側の値は同じ変更可能な場所を共有しません。parameterは最大4個です。再帰とcommand-line argumentはまだサポートしません。未対応の書き方は黙って別の意味にせず、診断します。
...
Primer v0.1には、一つの真偽値型、五つの数値型、固定長配列、ユーザーが定義する名前付きproduct typeがあります。

組み込み型名`bool`、`i32`、`u32`、`i64`、`f32`、`f64`と同名の関数や型は定義できません。定義位置でエラーになります。
...
現在、整数型の範囲と格納幅は別です。`i32`は`-2147483648`から`2147483647`、`u32`は`0`から`4294967295`までを表せますが、生成先ではどちらも64ビットの領域を使います。配列や構造体も現時点では32ビットへ詰めて格納しないため、`i64`から変更してもメモリ使用量は小さくなりません。
...
整数の接尾辞は`i32`、`u32`、`i64`です。接尾辞のない数字には、宣言・再代入・引数・戻り値・field・配列要素の期待型を伝えます。外側から期待型がなければ、同じ算術式の型が決まった値を手掛かりにします。
...
整数の`+`、`-`、`*`と、符号付き整数の単項`-`は、結果がその整数型の範囲を超えた場合に実行を停止します。範囲の端から反対側の値へ暗黙に折り返しません。整数の0による除算と、符号付き整数の最小値を`-1`で割る演算も停止します。`u32`への単項`-`は禁止し、`-0u32`もエラーです。整数除算は0方向へ丸めます。
...
現在は`i32`・`u32`・`i64`の全組み合わせで変換できます。数値を変えずに変換先で表せる場合だけ成功し、範囲外なら実行を停止します。切り捨てや折り返しはしません。整数と浮動小数点、`bool`、配列、構造体の間の変換は使えません。
...
Primer IRでは変換前後の整数型、入力、元の書式とソース位置を保持します。二つの書式は同じ種類の処理で、書式の違いは出自情報です。bytecodeには`convert.checked i32 -> u32`のように両方の型を出力し、対応するPrimer IRの`NodeId`と`Span`を命令の出自として保持します。C・LLVM・QBE・WAT・Windows x86-64では入力を64ビットで保持し、`i32`・`u32`への変換には変換先の範囲検査を生成します。`i64`への変換には追加の実行処理は不要ですが、Primer IRとbytecodeの明示変換は残ります。
