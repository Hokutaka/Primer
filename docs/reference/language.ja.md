...
組み込み型名`bool`、`i64`、`f32`、`f64`と同名の関数や型は定義できません。定義位置でエラーになります。
...
整数リテラルは、型が決まるまで10進数字の並びとして保持されます。型が`i64`に決まった時点で範囲を検査し、収まらない値はコンパイルエラーになります。符号は単項`-`として解析しますが、`-9223372036854775808`は`i64`の最小値として使用できます。
...
`i64`の`+`、`-`、`*`と単項`-`は、結果が`i64`の範囲を超えた場合に実行を停止します。範囲の端から反対側の値へ暗黙に折り返しません。整数の0による除算と、`i64`の最小値を`-1`で割る演算も停止します。
...
現在は`i64`から`i64`への変換だけをサポートします。数値と型は変わりませんが、明示的な変換としてPrimer IRとbytecodeに残ります。`i32`・`u32`や整数と浮動小数点の間の変換はまだ使えません。
...
Primer IRでは変換前後の整数型、入力、元の書式とソース位置を保持します。二つの書式は同じ種類の処理で、書式の違いは出自情報です。bytecodeには`convert.checked i64 -> i64`を出力し、対応するPrimer IRの`NodeId`と`Span`を命令の出自として保持します。C・LLVM・QBE・WAT・Windows x86-64では同型変換の実行処理を省略し、入力をそのまま使います。
