...
Primer v0.1には、一つの真偽値型、九つの数値型、固定長配列、ユーザーが定義する名前付きproduct typeがあります。

組み込み型名`bool`、`i8`、`u8`、`i16`、`u16`、`i32`、`u32`、`i64`、`f32`、`f64`と同名の関数や型は定義できません。定義位置でエラーになります。
...
現在は`i8`・`u8`・`i16`・`u16`・`i32`・`u32`・`i64`の全組み合わせで変換できます。数値を変えずに変換先で表せる場合だけ成功し、範囲外なら実行を停止します。切り捨てや折り返しはしません。整数と浮動小数点、`bool`、配列、構造体の間の変換は使えません。
...
両方の書式で、括弧内の式を一度だけ評価します。入力へ変換先の型を伝えて計算の意味を変えることはありません。入力が整数以外ならコンパイルエラーです。引数は一つ必要で、末尾のカンマは使用できます。変換は値を返す式なので、単独の文としては使いません。

入力側の計算で失敗した場合は、その計算の位置を報告します。例えば`i64(1 / 0)`は変換に到達する前に除算で停止します。

Primer IRでは変換前後の整数型、入力、元の書式とソース位置を保持します。二つの書式は同じ種類の処理で、書式の違いは出自情報です。bytecodeには`convert.checked i32 -> u32`のように両方の型を出力し、対応するPrimer IRの`NodeId`と`Span`を命令の出自として保持します。C・LLVM・QBE・WAT・Windows x86-64では入力を64ビットで保持し、8・16・32ビット整数への変換には変換先の範囲検査を生成します。`i64`への変換には追加の実行処理は不要ですが、Primer IRとbytecodeの明示変換は残ります。
