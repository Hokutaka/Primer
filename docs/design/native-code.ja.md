...
機械語の経路は言語の意味を実装し直さず、同じASMから構築します。今回の符号化・オブジェクト生成・リンクは外部ツールが担当します。Primer自身の命令エンコーダやリンカを追加したという意味ではありません。`emit-*`は引き続き成果物を返すだけで、外部ツールの起動は明示したスクリプトに分離します。
...
| `manifest.json` | スキーマ、ターゲット、ツールと版、実行引数、各段階の成否、成果物のSHA-256 |
