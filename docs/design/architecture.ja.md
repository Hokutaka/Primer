...
現在のコード生成バックエンドは、7種類の整数型を64ビットの値として格納します。符号なし整数も非負の64ビット値なので、64ビットの符号付き比較・除算で同じ数値上の意味を実現できます。8・16・32ビット整数の算術結果と変換結果には、元の型の範囲を検査する`CheckIntegerRange`をバックエンドIRへ残します。64ビットの演算自体にも従来の桁あふれ検査を適用します。
