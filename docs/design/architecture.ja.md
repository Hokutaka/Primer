...
AST、意味解析、Primer IRでは、整数型を共通の`IntegerType`で表します。現在登録されている種類は`I64`だけで、型名、符号の有無、値の範囲を決めるビット数を取得できます。各バックエンドへの変換ではこの種類を明示的に扱い、出力先の型へ対応付けます。整数型のビット数と、出力先での格納サイズやレジスタ幅は別の情報です。IRのテキスト表示は従来どおり`i64`です。
...
整数変換の`i64(value)`と`convert<i64>(value)`は同じ`ConvertInteger`へ解決し、元の書式は`ConversionSyntax`で別に保持します。入力の型を変換先から推論し直さず、入力を一度だけ評価します。現在は`i64`同士だけに対応し、bytecodeには出自付き変換命令を残します。各コード生成バックエンドでは、この同型変換に追加の実行命令は必要ありません。
