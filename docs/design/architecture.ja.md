...
AST、意味解析、Primer IR、bytecodeは整数の種類を共通の`IntegerType`で表します。`I32`・`U32`・`I64`を持ち、型名、符号、ビット数、最小値と最大値を取得できます。VMの値にも整数型を保持し、命令の型が一致することと結果の範囲を検査します。
...
整数変換の`i32(value)`と`convert<i32>(value)`は同じ`ConvertInteger`へ解決し、元の書式は`ConversionSyntax`で別に保持します。入力の型を変換先から推論し直さず、入力を一度だけ評価します。`i32`・`u32`・`i64`の全組み合わせを扱い、範囲外の変換は算術演算の桁あふれとは別のVMエラーになります。bytecodeの変換命令には変換前後の型と出自を残します。

現在のコード生成バックエンドは、三つの整数型を64ビットの値として格納します。`u32`も非負の64ビット値なので、64ビットの符号付き比較・除算で同じ数値上の意味を実現できます。`i32`・`u32`の算術結果と変換結果には、元の型の範囲を検査する`CheckIntegerRange`をバックエンドIRへ残します。64ビットの演算自体にも従来の桁あふれ検査を適用します。

この方針は値の範囲と失敗条件を実装するためのもので、32ビット格納や外部ABIへの対応ではありません。配列・構造体も64ビット領域を使うため、現段階ではメモリ使用量は減りません。Primer IRにある意味上の型と、バックエンドが選んだ格納型を混同しないことが重要です。
