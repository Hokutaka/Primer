...
`--run --expect-trap` requires a VM diagnostic and native SIGILL/Windows illegal-instruction termination. Successful execution, access violations, startup failures, and timeouts do not pass. Manifest outcomes distinguish `output-matched`, `expected-failure-confirmed`, `generated-not-executed`, and `failed`. Each tool has a 30-second timeout.

C u64 checks now emit reasons to stderr, which tests compare against the corresponding VM error category. Windows abort and unrelated fast-fail conditions can share an exit code, so the code alone is insufficient. See Microsoft's [abort](https://learn.microsoft.com/en-us/cpp/c-runtime-library/reference/abort) and [fast-fail](https://learn.microsoft.com/en-us/cpp/intrinsics/fastfail) specifications. QBE requires SIGABRT, LLVM/assembly illegal-instruction termination, and WAT unreachable. Detailed per-condition native diagnostics for trap-only routes remain future work.
