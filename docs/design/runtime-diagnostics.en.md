...
The [file-identity API](source-files.en.md) produces records such as `primer: runtime-v1 code=division-by-zero node=1 file=2 bytes=6..11`, with a range local to that file. Existing single-source APIs and CLI inputs without import/pub retain their records. Readers must support this optional field; older strict readers cannot accept registered-file records. The bundled observation script and WAT validation host validate both forms, rejecting `file=0`, leading zeros, duplicate fields, and invalid numbers. CLI inputs using [modules](modules.en.md) report file IDs, including the entry, resolved against the inventory and text from `emit-sources`.
