...
2026-09-08時点、PR #44〜#48を統合した実装を基準に整理します。「現在」は実装済み、「候補」は設計・実装前の提案です。候補の構文や採用を確定する文書ではありません。現在の正確な仕様は[言語リファレンス](../reference/language.ja.md)を参照してください。
...
| 関数と制御構文 | 型付き引数・戻り値、`void`、`if`・`else`、`while`・`for`、`break`・`continue`・`return` | 関数は最大4引数。再帰なし。モジュールなし | [function_values](../../examples/function_values.prim)、[loop_control](../../examples/loop_control.prim) |
...
言語機能を扱えることと、観測の詳しさが等しいことは別です。VMには停止理由とソース位置の診断がありますが、生成経路ではtrapやabortだけになる場合があり、全経路での実行時診断の統一は未完了です。LLVM/ASMの出自注釈も、全経路・全最適化段階のデバッグ情報を保証するものではありません。

## 次に持つべきもの：提案する順序

| 優先 | 項目 | なぜ必要か・最初の確認例 |
| --- | --- | --- |
| 1 | 停止理由・ソース位置・実行成否の共通契約 | 桁あふれ、ゼロ除算、変換失敗、境界外アクセスを区別し、どの式で止まったか追う。同じ失敗例をVMと生成経路で比較する |
| 2 | コードを分ける仕組み | モジュール、名前空間、公開範囲、明示的なコンパイル時定数を設計する。統計計算の関数と定数を別ファイルへ移しても意味が変わらない例から始める |
...
この順序は全項目を一括実装する約束ではありません。まず優先1を具体化し、各追加機能は小さい設計と実行例で採否を決めます。GPUの限定実験にモジュール・ジェネリクス・動的確保の全実装を必須とはしません。

優先1の最初の実装として、VMとWindows/LinuxのASM・自前オブジェクトに[実行時停止の共通記録](runtime-diagnostics.ja.md)を追加しました。停止前の出力も保持します。C・LLVM・QBE・WATへの展開は残っています。
