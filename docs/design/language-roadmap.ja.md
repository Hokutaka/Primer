...
優先1の最初の実装として、VMとWindows/LinuxのASM・自前オブジェクトに[実行時停止の共通記録](runtime-diagnostics.ja.md)を追加しました。停止前の出力も保持します。C・LLVM・QBE・WATへの展開も実装し、同じ停止理由・ソース位置・先行出力を比較します。言語機能として次に具体化する候補は、この表のモジュールと名前空間です。

モジュール化の最初の段階として、[ファイルを識別できるソース位置](source-files.ja.md)を実装しました。次は明示的なimport・名前空間・公開範囲、その後にCLIからの分割プログラムの全経路比較へ進みます。現在の分割例はRust APIによる出自検証であり、モジュール構文は未実装です。
