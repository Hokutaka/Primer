...
- `break;` and `continue;` target only the innermost loop.
...
`for`, `foreach`, `switch`, `break`, and `continue` are not implemented. Whether a value-producing form of `if` should be added later remains open.
